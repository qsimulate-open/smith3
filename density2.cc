...
OutStream Density2::create_target(const string indent, const int i) const {
  OutStream out;

  out.tt << "class Task" << i << " : public Density2Task {" << endl;
  out.tt << "  protected:" << endl;
  out.tt << "    std::shared_ptr<Tensor> d2_;" << endl;
  out.tt << "    IndexRange closed_;" << endl;
  out.tt << "    IndexRange active_;" << endl;
  out.tt << "    IndexRange virt_;" << endl;
  out.tt << "" << endl;
  out.tt << "    void compute_() {" << endl;
  out.tt << "      d2_->zero();" << endl;
  out.tt << "    };" << endl;
  out.tt << "" << endl;
  out.tt << "  public:" << endl;
  ...
  out.ss << "      auto density2_ = std::make_shared<Queue>();" << endl;
  out.ss << indent << "std::vector<std::shared_ptr<Tensor>> tensor" << i << " = {Den1};" << endl;
  out.ss << indent << "auto task" << i << " = std::make_shared<Task" << i << ">(tensor" << i << ");" << endl;
  out.ss << indent << "density2_->add_task(task" << i << ");" << endl << endl;
...
OutStream Density2::generate_task(const string indent, const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  OutStream out;
  ...
  if (parent_) {
    ...
    out.ss << indent << "task" << ic << "->add_dep(task" << iz << ");" << endl;
  } else {
    assert(depth() == 0);
    out.ss << indent << "task" << ic << "->add_dep(task" << iz << ");" << endl;
  }
  out.ss << indent << "density2_->add_task(task" << ic << ");" << endl;
  out.ss << endl;
  return out;
}


OutStream Density2::generate_compute_header(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors, const bool no_outside) const {
  ...
  OutStream out;
  out.tt << "class Task" << ic << " : public Density2Task {" << endl;
  out.tt << "  protected:" << endl;
...
OutStream Density2::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
...
OutStream Density2::generate_bc(const string indent, const shared_ptr<BinaryContraction> i) const {
  OutStream out;
