...
pair<string, string> Density2::create_target(const string indent, const int i) const {
  ...
  tt << "  protected:" << endl;
  tt << "    std::shared_ptr<Tensor> d2_;" << endl;
  ...
  ss << "      std::shared_ptr<Queue> density2_(new Queue());" << endl;
...
string Density2::generate_task(const string indent, const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  stringstream ss;
...
string Density2::generate_compute_header(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors, const bool no_outside) const {
  ...
  stringstream tt;
  tt << "class Task" << ic << " : public Density2Task {" << endl;
  tt << "  protected:" << endl;
...
tuple<string,string> Density2::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
  ...
  cc << "Task" << ic << "::Task" << ic << "(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range" << (need_e0 ? ", double e" : "" ) <<  ") {" << endl;
