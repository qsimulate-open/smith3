...
OutStream Tree::create_target(const int i) const {
  ...
  if (!is_energy_tree()) {
    ...
    out.tt << "    std::shared_ptr<TATensor<" << DataType << "," << target_index_size() << ">> tensor_;" << endl;
    out.tt << "    const bool reset_;" << endl;
    out.tt << "" << endl;
    out.tt << "    void compute_() {" << endl;
    out.tt << "      if (reset_) tensor_->zero();" << endl;
    out.tt << "    }" << endl;
    out.tt << "" << endl;
    out.tt << "  public:" << endl;
    out.tt << "    Task" << i << "(std::shared_ptr<TATensor<" << DataType << "," << target_index_size() << ">> t, const bool reset) : tensor_(t), reset_(reset) { }" << endl;
    out.tt << "};" << endl << endl;
...
OutStream Tree::generate_task(const int ic, const vector<shared_ptr<Tensor>> op, const list<shared_ptr<Tensor>> g, const int i0, const bool diagonal) const {
  ...
  tmp << indent << "  " << (diagonal ? "" : "auto ") << "task" << ic
                << " = make_shared<Task" << ic << ">(" << merge__(ops, label_) << (scalar.empty() ? "" : ", this->e0_") << ");" << endl;
...
OutStream Tree::generate_task(const int ic, const vector<string> ops, bool der) const {
  assert(ops.front().find("Gamma") != string::npos);
  OutStream out;
  out.gg << "  auto task" << ic
         << " = make_shared<Task" << ic << ">(array<shared_ptr<Tensor>," << count_distinct_tensors__(ops) << ">{{"
         << merge2__(ops, label_) << "}}" << (der ? ", cindex" : ", pindex") << ");" << endl;
  return out;
}
...
OutStream Tree::generate_compute_operators(shared_ptr<Tensor> target, const vector<shared_ptr<Tensor>> op, const bool dagger) const {
  ...
  int uniq_cnt = 1;
  for (auto& i : op) {
    const string label = label__(i->label());
    ...
    if (root_targets()) {
      ...
      for (auto& j : bc_) {
        ...
        if (depth() == 0 ) {
          ...
          // virtual
          assert(!is_energy_tree());
          auto proj_tensor = make_shared<Tensor>(1.0, "r", dm);
...
OutStream Tree::generate_compute_header(const int ic, const vector<shared_ptr<Tensor>> tensors) const {
  ...
  for (auto& i : tensors) {
    string lab = i->label();
    if (lab.find("dagger") != std::string::npos)
      lab = string(lab.begin(), lab.begin() + lab.find("dagger"));
    if (ranks.find(lab) == ranks.end()) {
      labels.push_back(lab);
      ranks.emplace(lab, (depth() != 0 ? i->index().size() : target_index_size()));
    }
  }
