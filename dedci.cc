...
tuple<string, string, string> Dedci::create_target(const string indent, const int i) const {
  stringstream ss, tt, cc;

  tt << "class Task" << i << " : public DedciTask {" << endl;
  tt << "  protected:" << endl;
  tt << "    std::shared_ptr<Tensor> dec_;" << endl;
  ...
  ss << "      auto dedci_ = std::make_shared<Queue>();" << endl;
  ss << indent << "std::vector<std::shared_ptr<Tensor>> tensor" << i << " = {deci};" << endl;
...
string Dedci::generate_task(const string indent, const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  ...
  ss << indent << "auto task" << ic << " = std::make_shared<Task" << ic << ">(tensor" << ic << ", cindex" << (scalar.empty() ? "" : ", this->e0_") << ");" << endl;
  if (parent_) {
...
string Dedci::generate_compute_header(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors, const bool no_outside) const {
  ...
  stringstream tt;
  tt << "class Task" << ic << " : public DedciTask {" << endl;
  tt << "  protected:" << endl;
...
tuple<string,string> Dedci::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
  ...
  tt << "  public:" << endl;
  tt << "    Task" << ic << "(std::vector<std::shared_ptr<Tensor>> t, std::array<std::shared_ptr<const IndexRange>,4> range" << (need_e0 ? ", double e" : "" ) <<  ");" << endl;

  cc << "Task" << ic << "::Task" << ic << "(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range" << (need_e0 ? ", double e" : "" ) <<  ") {" << endl;
  ...
  // add subtasks
  if (!ti.empty()) {
    cc << indent  << "subtasks_.push_back(make_shared<Task_local>(std::array<const Index," << ti.size() << ">{{";
...
pair<string, string> Dedci::generate_bc(const string indent, const shared_ptr<BinaryContraction> i) const {
  stringstream ss;
  stringstream tt;
