...
int main() {
  ...
  shared_ptr<Equation> eq2(new Equation(theory, "rc", {dum, proj_list, H}));
  shared_ptr<Equation> eq2a(new Equation(theory, "rd", {dum, proj_list, hc}, 2.0));
  eq0->merge(eq1);
  eq0->merge(eq2);
  eq0->merge(eq2a);
  ...
  shared_ptr<Equation> eq3(new Equation(theory, "ec", {dum, t_dagger, H}, 0.25));
  shared_ptr<Equation> eq3a(new Equation(theory, "ed", {dum, t_dagger, hc}, 0.5));
  eq3->merge(eq3a);
  eq3->set_tree_type("energy");
  cout << eq3->generate();

  // generate Norm <1|1> to be used in various places
  shared_ptr<Equation> eq5(new Equation(theory, "ca", {dum, t_dagger, t_list}, 0.25));
  eq5->set_tree_type("energy", "corr");
  cout << eq5->generate();

  // density matrix equations //
  // one-body contribution d2
  shared_ptr<Equation> eq6(new Equation(theory, "da", {dum, t_dagger, ex1b, t_list}, 0.25));
  eq6->set_tree_type("residual", "density");
  cout << eq6->generate();
  // one-body contribution d1
  shared_ptr<Equation> eq6a(new Equation(theory, "db", {dum, ex1b, t_list}, 0.5));
  eq6a->set_tree_type("residual", "density1");
  cout << eq6a->generate();

  // two-body contribution D1
  shared_ptr<Equation> eq7(new Equation(theory, "d2a", {dum, proj_list, t_list}, 0.5));
  ...
  shared_ptr<Equation> eq4(new Equation(theory, "dedcia", {dum, t_dagger, f, t_list}, 0.5, make_pair(true, false)));
//shared_ptr<Equation> eq4a(new Equation(theory, "dedcib", {dum, t_dagger, f, t_list}, 0.25, make_pair(false, true)));
  shared_ptr<Equation> eq4b(new Equation(theory, "dedcic", {dum, t_dagger, t_list}, -0.5, "e0", make_pair(true, false)));
//shared_ptr<Equation> eq4c(new Equation(theory, "dedcid", {dum, t_dagger, t_list}, -0.25, "e0", make_pair(false, true)));
  shared_ptr<Equation> eq4d(new Equation(theory, "dedcie", {dum, t_dagger, H}, 0.50, make_pair(true, false)));
  shared_ptr<Equation> eq4e(new Equation(theory, "dedcif", {dum, t_dagger, H}, 0.50, make_pair(false, true)));
  shared_ptr<Equation> eq4f(new Equation(theory, "dedcig", {dum, t_dagger, hc}, 1.00, make_pair(true, false)));
  shared_ptr<Equation> eq4g(new Equation(theory, "dedcih", {dum, t_dagger, hc}, 1.00, make_pair(false, true)));
//eq4->merge(eq4a);
  eq4->merge(eq4b);
//eq4->merge(eq4c);
  eq4->merge(eq4d);
