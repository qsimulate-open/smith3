...
int main() {
  ...
  shared_ptr<Equation> eq4b(new Equation("dedcic", {dum, t_dagger, hc}, scale*0.25, make_pair(true, false)));
  shared_ptr<Equation> eq4c(new Equation("dedcid", {dum, t_dagger, hc}, scale*0.25, make_pair(false, true)));
  eq4->merge(eq4a);
  eq4->merge(eq4b);
  eq4->merge(eq4c);
#endif
