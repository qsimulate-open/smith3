...
OutStream Density::create_target(const int i) const {
  OutStream out;
  ...
  out.ee << "  auto density_ = make_shared<Queue>();" << endl;
  out.ee << "  vector<shared_ptr<Tensor>> tensor" << i << " = {den2};" << endl;
  out.ee << "  auto task" << i << " = make_shared<Task" << i << ">(tensor" << i << ");" << endl;
  out.ee << "  density_->add_task(task" << i << ");" << endl << endl;
...
OutStream Density::generate_task(const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  OutStream out;
  ...
  out.ee << "  density_->add_task(task" << ic << ");" << endl;
  out.ee << endl;
  return out;
}
