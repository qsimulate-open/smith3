...
tuple<string, string, string> Density::create_target(const string indent, const int i) const {
  stringstream ss, tt, cc;

  tt << "class Task" << i << " : public DensityTask {" << endl;
  tt << "  protected:" << endl;
  tt << "    std::shared_ptr<Tensor> d_;" << endl;
...
string Density::generate_compute_header(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors, const bool no_outside) const {
  ...
  stringstream tt;
  tt << "class Task" << ic << " : public DensityTask {" << endl;
  tt << "  protected:" << endl;
...
tuple<string,string> Density::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
