...
OutStream Density::create_target(const string indent, const int i) const {
  ...
  out.ss << "      auto density_ = std::make_shared<Queue>();" << endl;
  out.ss << indent << "std::vector<std::shared_ptr<Tensor>> tensor" << i << " = {den2};" << endl;
  out.ss << indent << "auto task" << i << " = std::make_shared<Task" << i << ">(tensor" << i << ");" << endl;
  out.ss << indent << "density_->add_task(task" << i << ");" << endl << endl;
...
OutStream Density::generate_task(const string indent, const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  OutStream out;
  ...
  out.ss << indent << "density_->add_task(task" << ic << ");" << endl;
  out.ss << endl;
  return out;
}


OutStream Density::generate_compute_header(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors, const bool no_outside) const {
...
OutStream Density::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
...
OutStream Density::generate_bc(const string indent, const shared_ptr<BinaryContraction> i) const {
  OutStream out;
