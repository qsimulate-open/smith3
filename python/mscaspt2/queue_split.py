...
def insert():
    return "#include <src/smith/caspt2/MSCASPT2_tasks.h>\n"
...
for line in lines:
    if (len(line) >= 17 and (line[0:17] == "shared_ptr<Queue>" or line[0:21] == "MSCASPT2::MSCASPT2::M")):
        if (tmp != ""):
            tasks.append(tmp)
            tmp = ""
    tmp += line + "\n"
...
os.remove("MSCASPT2.cc")

fout = open("MSCASPT2.cc", "w")
