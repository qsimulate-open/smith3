...
for i in range(len(tasks)):
    if (num != 0 and num % chunk == 0):
        n = num / chunk
        fout = open("RelMSCASPT2_tasks" + str(n) + ".h", "w")
...
os.remove("RelMSCASPT2_tasks.h")
fout = open("RelMSCASPT2_tasks.h", "w")
out = header2
for i in range(n+1):
    if (i > 0):
        out += "#include <src/smith/caspt2/RelMSCASPT2_tasks" + str(i) + ".h>\n"
out += footer2
fout.write(out)
fout.close()
