...
pair<string, string> Residual::create_target(const string indent, const int i) const {
  ...
  tt << "  protected:" << endl;
  tt << "    std::shared_ptr<Tensor> r_;" << endl;
...
tuple<string,string> Residual::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
