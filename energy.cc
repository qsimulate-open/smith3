...
OutStream Energy::generate_task(const string indent, const int ip, const int ic, const vector<string> op, const string scalar, const int i0, bool der) const {
  OutStream out;
  ...
  out.ss << indent << "energy_->add_task(task" << ic << ");" << endl;
  out.ss << endl;
  return out;
}
...
OutStream Energy::generate_bc(const string indent, const shared_ptr<BinaryContraction> i) const {
  OutStream out;
  ...
  if (depth() != 0) {
    ...
    // call dgemm
    {
      ...
      if (t0.first != "" || t1.first != "") {
        ...
        string ss0 = t1.second== "" ? "1" : t1.second;
        out.tt << dindent << "energy_ += ddot_(" << ss0 << ", i0data_sorted, 1, i1data_sorted, 1);" << endl;
      }
    }
