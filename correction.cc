...
OutStream Correction::generate_bc(const string indent, const shared_ptr<BinaryContraction> i) const {
  OutStream out;
  ...
  if (depth() != 0) {
    ...
    // call dgemm
    {
      ...
      if (t0.first != "" || t1.first != "") {
        ...
        string ss0 = t1.second== "" ? "1" : t1.second;
        out.tt << dindent << "correction_ += ddot_(" << ss0 << ", i0data_sorted, 1, i1data_sorted, 1);" << endl;
      }
    }
