...
pair<string, string> Density1::create_target(const string indent, const int i) const {
  stringstream ss;
  stringstream tt;
  tt << "class Task" << i << " : public Density1Task {" << endl;
  tt << "  protected:" << endl;
  tt << "    std::shared_ptr<Tensor> d1_;" << endl;
  ...
  tt << "" << endl;
  tt << "  public:" << endl;
  tt << "    Task" << i << "(std::vector<std::shared_ptr<Tensor>> t) : Density1Task() {" << endl;
  tt << "      d1_ =  t[0];" << endl;
  tt << "    }" << endl;
  tt << "    ~Task" << i << "() {}" << endl;
  tt << "};" << endl << endl;

  ss << "      auto density1_ = std::make_shared<Queue>();" << endl;
  ss << indent << "std::vector<std::shared_ptr<Tensor>> tensor" << i << " = {den1};" << endl;
  ss << indent << "auto task" << i << " = std::make_shared<Task" << i << ">(tensor" << i << ");" << endl;
  ss << indent << "density1_->add_task(task" << i << ");" << endl << endl;

  return make_pair(ss.str(), tt.str());
}


shared_ptr<Tensor> Density1::create_tensor(list<shared_ptr<const Index>> dm) const {
 return make_shared<Tensor>(1.0, "den1", dm);
}


string Density1::generate_task(const string indent, const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  stringstream ss;
  ...
  ss << indent << "density1_->add_task(task" << ic << ");" << endl;
  ss << endl;
  return ss.str();
}


string Density1::generate_compute_header(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors, const bool no_outside) const {
  ...
  stringstream tt;
  tt << "class Task" << ic << " : public Density1Task {" << endl;
  tt << "  protected:" << endl;
...
tuple<string,string> Density1::generate_compute_footer(const int ic, const list<shared_ptr<const Index>> ti, const vector<shared_ptr<Tensor>> tensors) const {
  const int ninptensors = tensors.size()-1;
  ...
  tt << "  public:" << endl;
  tt << "    Task" << ic << "(std::vector<std::shared_ptr<Tensor>> t, std::array<std::shared_ptr<const IndexRange>,3> range" << (need_e0 ? ", double e" : "" ) <<  ");" << endl;

  cc << "Task" << ic << "::Task" << ic << "(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range" << (need_e0 ? ", double e" : "" ) <<  ") : Density1Task() {" << endl;
...
pair<string, string> Density1::generate_bc(const string indent, const shared_ptr<BinaryContraction> i) const {
  stringstream ss;
  stringstream tt;
    ...
    if (depth() != 0) {
      ...
      throw logic_error("shouldn't happen in Density1::generate_bc");
    }
