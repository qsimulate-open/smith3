...
OutStream Density1::create_target(const int i) const {
  OutStream out;
  ...
  out.ee << "  auto density1_ = make_shared<Queue>();" << endl;
  out.ee << "  vector<shared_ptr<Tensor>> tensor" << i << " = {den1};" << endl;
  out.ee << "  auto task" << i << " = make_shared<Task" << i << ">(tensor" << i << ");" << endl;
  out.ee << "  density1_->add_task(task" << i << ");" << endl << endl;
...
OutStream Density1::generate_task(const int ip, const int ic, const vector<string> op, const string scalar, const int iz, bool der) const {
  OutStream out;
